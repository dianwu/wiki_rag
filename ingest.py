...
def main():
    """
    Main function to run the data ingestion and vectorization pipeline.
    """
    # --- Configuration ---
    DATA_DIR = "data"
    ...
    if Path(PERSIST_DIR).exists():
        logging.warning(f"Vector store at '{PERSIST_DIR}' already exists. ")
        logging.warning("To re-ingest data, please delete the directory first.")
        return
    ...
    if processed_documents:
        ...
        logging.error("Data processing failed or produced no documents. Vector store not created.")
